...
Pages.dash = function () {
  ...
  if (State.profile === 'funcionario') {
    const minhas   = State.demandas.filter(d => d.autor === p.name);
    const abertas  = minhas.filter(d => d.status !== 'Resolvida').length;
    const recentes = minhas.slice(0, 2);

    return `
      <div class="pg-header">
        <div class="pg-title">Bom dia, ${p.name.split(' ')[0]} 👋</div>
        <div class="pg-sub">${abertas > 0 ? `Você tem ${abertas} demanda${abertas > 1 ? 's' : ''} em aberto · ` : ''}${hoje}</div>
      </div>
      <div class="grid4">
        <div class="metric">       <div class="mlabel">Demandas abertas</div><div class="mval blue">${abertas}</div><div class="msub">${abertas === 1 ? '1 aguarda retorno' : abertas + ' aguardando retorno'}</div></div>
        <div class="metric green"> <div class="mlabel">Resolvidas (30d)</div><div class="mval green">${minhas.filter(d=>d.status==='Resolvida').length}</div><div class="msub">Dentro do prazo</div></div>
      ...
      <div class="grid2">
        <div class="card">
          ...
          ${recentes.length > 0 ? recentes.map(d => `
            <div class="trow hover-row"
                 onclick="Pages._openDemanda('${d.id}')"
                 onkeydown="if(event.key==='Enter'||event.key===' '){Pages._openDemanda('${d.id}');}"
                 role="button" tabindex="0" aria-label="Demanda ${d.id}: ${d.titulo}">
              <div style="flex:1">
            ...
            </div>`).join('') : `<div style="padding:20px 0;text-align:center;color:var(--text3);font-size:13px">Nenhuma demanda recente</div>`}
          <div class="mt12">
        ...
        <div class="card">
          ...
          <div class="alert-box a-info mt12">
            <div class="dot d-blue" aria-hidden="true"></div>
            <div>
  ...
  if (State.profile === 'gestor') return `
    <div class="pg-header">
      <div class="pg-title">Painel — ${p.setor}</div>
    ...
    <div class="grid2">
        ...
        ${[['Rampas e acessos',72],['Intérprete LIBRAS',45],['Equipamentos',38],['Documentação',28],['Infraestrutura',17],['Outros',12]].map(([l,v]) => `
          <div style="margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2);margin-bottom:4px">
              <span>${l}</span>
              <span style="font-weight:700;color:var(--brand-primary)">${v}%</span>
            </div>
            <div class="bar-bg" role="progressbar" aria-valuenow="${v}" aria-valuemin="0" aria-valuemax="100" aria-label="${l}: ${v}%">
      ...
      <div class="card">
        ...
        <div class="alert-box a-crit">
          <div class="dot d-red" aria-hidden="true"></div>
          <div>
            <div style="font-size:12px;font-weight:700;color:var(--text)">🔴 Elevador B2 fora de operação</div>
            <div style="font-size:11px;color:var(--text2)">Sensor IoT detectou falha há 40min.</div>
            <button class="btn btn-ghost mt8" style="font-size:11px;padding:4px 10px" onclick="State.goTo('alertas')">Ver detalhe →</button>
          </div>
        </div>
        <div class="alert-box a-warn">
          <div class="dot d-warn" aria-hidden="true"></div>
          <div>
            <div style="font-size:12px;font-weight:700;color:var(--text)">🟡 Rampa C3 — obstáculo detectado</div>
            <div style="font-size:11px;color:var(--text2)">IA identificou bloqueio por foto há 1h12min.</div>
          </div>
        </div>
        <div class="alert-box a-ok">
          <div class="dot d-green" aria-hidden="true"></div>
          <div>
            <div style="font-size:12px;font-weight:700;color:var(--text)">✅ Banheiro adaptado A1 — OK</div>
            <div style="font-size:11px;color:var(--text2)">Vistoria confirmada hoje às 08:14.</div>
          </div>
        </div>
      </div>
    </div>`;
        ...
        ${[['Física / motora',61,'green'],['Visual',38,''],['Auditiva — LIBRAS',29,''],['Intelectual',18,''],['Múltipla',14,'']].map(([l,v,c]) => `
          <div style="margin-bottom:12px">
            <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text2)">
              <span>${l}</span>
              <span style="font-weight:700">${v} func.</span>
            </div>
            <div class="bar-bg" role="progressbar" aria-valuenow="${Math.round(v/61*100)}" aria-valuemin="0" aria-valuemax="100" aria-label="${l}: ${v} funcionários">
              <div class="bar-fill ${c}" style="width:${Math.round(v/61*100)}%"></div>
            </div>
          </div>`).join('')}
      </div>
      <div class="card">
        ...
        <div class="mt12">
          <button class="btn btn-primary" onclick="State.goTo('relatorios')" style="width:100%;justify-content:center">📈 Ver relatório completo</button>
        </div>
      </div>
    </div>`;
  ...
  return `
    <div class="pg-header">
      ...
      <div class="pg-sub">Status em tempo real · ${hora} · Administrador</div>
    </div>
    <div class="grid4">
    ...
    <div class="grid2">
      <div class="card">
        ...
        ${[['API backend (Laravel)','Operacional','t-ok'],['Microsserviço IA — visão','Operacional','t-ok'],['STT Whisper / TTS','Operacional','t-ok'],['WebSocket / alertas','Operacional','t-ok'],['MQTT broker — sensores','1 falha ativa','t-warn'],['Banco PostgreSQL','Operacional','t-ok'],['Redis / cache','Operacional','t-ok']].map(([l,s,t]) => `
          <div class="trow">
            <div style="flex:1;font-size:12px;color:var(--text)">${l}</div>
      ...
      <div class="card">
        ...
        ${[['IA classificou 12 demandas automaticamente','há 2 min'],['Alerta: sensor E-07 offline (Elevador B2)','há 8 min'],['Relatório mensal gerado pelo RH','há 31 min'],['Novo usuário criado: juliana.costa','há 1h'],['Backup automático concluído','há 3h']].map(([a,t]) => `
          <div class="trow">
            <div style="flex:1;font-size:12px;color:var(--text)">${a}</div>
            <span style="font-size:10px;color:var(--text3);flex-shrink:0">${t}</span>
          </div>`).join('')}
      </div>
    </div>`;
};
